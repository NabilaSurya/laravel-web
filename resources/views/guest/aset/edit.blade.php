                ...
                <form action="{{ route('aset.update', $aset) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select
                            ...
                            @foreach ($kategori as $kat)
                                @php
                                    // Mengambil nilai lama dari old() atau nilai dari $aset
                                    $selected =
                                        old('kategori_id', $aset->kategori_id) == $kat->kategori_id ? 'selected' : '';
                                @endphp
                                <option value="{{ $kat->kategori_id }}" {{ $selected }}>
                                    {{ $kat->nama }} ({{ $kat->kode }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                            ...
                            id="kode_aset" name="kode_aset" value="{{ old('kode_aset', $aset->kode_aset) }}" required>
                        @error('kode_aset')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select
                            ...
                            @foreach (['Baik', 'Rusak Ringan', 'Rusak Berat'] as $kondisi)
                                @php
                                    $selected = old('kondisi', $aset->kondisi) == $kondisi ? 'selected' : '';
                                @endphp
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button type="submit"
                            ...
                            <i class="fa fa-upload mr-1"></i> Update Aset
                        </button>
                    </div>
                </form>
            </div>
